...
class common {
			...
			uni.$u.http
			 	...
			 	.then(res => {
					res.rows.forEach((item,index)=>{
								login.groupIds.push(item.group_id)
							})
							...
							{userid: this.user.id},
							{
								custom: { auth: true }
								 ...
								 if(res.rows){
									...
									uni.$emit('onUpdateGroupList', res.rows)
								 	this.addChatDetail(message, false)
								 	this.updateSessionList(message, false)
								 	uni.$emit('onMessage', message)
								 	if (vibrateLong) {
	...
	uni.$u.http
	.post(
		'/message/updateNoReadGroupHistory',
		{ids: ids},
		{
			custom: { auth: true }
		}
	...
	send(message, onProgress = false) {
		if(message.chat_type === 'group'){
			console.log(message)
					...
					if(group && group.length != 0){
						group.group_users.forEach((item,index)=>{
							...
							detailId.userId = item.id;
							detailId.create_time = (new Date()).getTime();
							detailIds.push(detailId)
						})
						console.log(detailIds)
						uni.$u.http.post('/chat/addGroupChatDetailIds', {
			...
			let friends = $store.state.user.friendIds;
			let friendids = [];
			if(friends.length){
			...
			if	(friendids.indexOf(message.to_id) === -1){
				uni.showToast({
					title: "你们还不是好友",
					duration: 2000,
					icon: 'none'
	...
	addFriendApply(message) {
		let key = `friendApply_${this.user.id}`
	...
	addChatDetail(message, isSend = true) {
		...
		let key = `chatDetail_${this.user.id}_${id}`
		// 获取原来的聊天记录
	...
	getChatDetail(key = false, id = 0 , page = 1 , limit = 20) {
		key = key ? key : `chatDetail_${this.user.id}_${id}`;
	...
	addChatGroup(group,users) {
		...
		console.log(group,5678)
		var that = this;
		uni.$u.http.post('/contact/createcommunity', {
				admin: group.admin,
			...
			if (res.code !== -1) {
				...
				uni.$u.http.post('/contact/createGroupUser', {
